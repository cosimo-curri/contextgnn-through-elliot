...
class DataSet(AbstractDataset):
    ...
    def get_relbench_df(self):
        src_df = pd.read_parquet(os.path.join(os.path.dirname(self.config.data_config.train_path), 'src_df.tsv'))
        dst_df = pd.read_parquet(os.path.join(os.path.dirname(self.config.data_config.train_path), 'dst_df.tsv'))
        target_df = pd.read_parquet(os.path.join(os.path.dirname(self.config.data_config.train_path), 'target_df.tsv'))
        train_df = pd.read_parquet(os.path.join(os.path.dirname(self.config.data_config.train_path), 'train_df.tsv'))
        test_df = pd.read_parquet(os.path.join(os.path.dirname(self.config.data_config.train_path), 'test_df.tsv'))

        src_df['user_id'] = src_df['user_id'].map(self.public_users)

        dst_df['item_id'] = dst_df['item_id'].map(self.public_items)

        train_df['user_id'] = train_df['user_id'].map(self.public_users)
        train_df['item_id'] = train_df['item_id'].map(lambda x: [self.public_items[el] for el in x])

        test_df['user_id'] = test_df['user_id'].map(self.public_users)
        test_df['item_id'] = test_df['item_id'].map(lambda x: [self.public_items[el] for el in x])

        target_df['user_id'] = target_df['user_id'].map(self.public_users)
        target_df['item_id'] = target_df['item_id'].map(self.public_items)

        src_df['pseudo_time'] = pd.to_datetime(src_df['pseudo_time'], unit='s')
        dst_df['pseudo_time'] = pd.to_datetime(dst_df['pseudo_time'], unit='s')
        target_df['pseudo_time'] = pd.to_datetime(target_df['pseudo_time'], unit='s')
        train_df['pseudo_time'] = pd.to_datetime(train_df['pseudo_time'], unit='s')
        test_df['pseudo_time'] = pd.to_datetime(test_df['pseudo_time'], unit='s')
        return src_df, dst_df, target_df, train_df, test_df
