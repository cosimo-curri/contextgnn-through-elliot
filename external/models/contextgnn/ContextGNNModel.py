...
from .nn.models import HeteroGraphSAGE
...
class ContextGNNModel(RHSEmbeddingGNN):
    ...
    def forward(
        ...
        if self.lhs_embedding is not None:
            lhs_embedding = self.lhs_embedding().to(self.device)[batch[entity_table].n_id]
            x_dict[entity_table] = lhs_embedding

        # Add ID-awareness to the root node
        x_dict[entity_table][:seed_time.size(0
                                             )] += self.id_awareness_emb.weight.to(self.device)
        ...
        if not self.is_static:
            ...
            for node_type, rel_time in rel_time_dict.items():
                x_dict[node_type] = x_dict[node_type].to(self.device) + rel_time
